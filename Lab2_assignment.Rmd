...
author: "Assignment created by: Matt Harris // completed by: Alex Cartwright"
date: "9/10/2021"
...
ggplot()+
  geom_sf(data = acsTractsPHL.2019.sf, aes(fill = renterPct.2019),
          color = "transparent")+
  scale_fill_viridis()+
  geom_sf(data = acsTractsPHL.2019.sf %>%
            filter(Neighborhood == "SPRUCE HILL") %>%
            st_union(),
          color = "white",
          fill = "transparent")+
  # Bonus to figure out different color ramps with scale_fill_viridis()
  labs(
    title = "Philadelphia Housing Trends",
    subtitle = "Who is renting in Philly?",
    caption = "Percentage of occupied housing units under tenancy (by census tract)")
